...
class AttendanceExportSingle implements FromView, ShouldAutoSize
{
    /**
    ...
    */
    ...
    public function view(): View{
        ...
        $user = MProf::where('user_id', $this->uid)->first();
        
        $final = [];
        $period = CarbonPeriod::create($minTime, $maxTime);
        $dates = $period->toArray();
        $attendances = MAttend::where('mprof_id', $this->uid)->whereBetween('date', [$minTime->format('Y-m-d'), $maxTime->format('Y-m-d')])->where('m_attends.in_time', '!=', '00:00:00')->orWhere('m_attends.late_time', '!=', '00:00:00')->orWhere('m_attends.in_tolerance_time', '!=', '00:00:00')->join('m_profs', 'm_profs.id', '=', 'm_attends.mprof_id')->get();
        //dd($attendances);
        ...
        $attend = [];
        $in = MAttend::where('mprof_id', $this->uid)->whereBetween('date', [$minTime->format('Y-m-d'), $maxTime->format('Y-m-d')])->where('mprof_id', $user->id)->where('in_time', '!=', '00:00:00')->orWhere('in_tolerance_time', '!=', '00:00:00')->where('mprof_id', $user->id)->join('m_profs', 'm_profs.id', '=', 'm_attends.mprof_id')->join('tmtables', 'tmtables.id', '=', 'm_attends.tmtable_id')->orderBy('m_profs.name', 'ASC')->get();
        $late = MAttend::where('mprof_id', $this->uid)->whereBetween('date', [$minTime->format('Y-m-d'), $maxTime->format('Y-m-d')])->where('mprof_id', $user->id)->where('late_time', '!=', '00:00:00')->join('m_profs', 'm_profs.id', '=', 'm_attends.mprof_id')->join('tmtables', 'tmtables.id', '=', 'm_attends.tmtable_id')->orderBy('m_profs.name', 'ASC')->get();
        $permit = MAttend::where('mprof_id', $this->uid)->whereBetween('date', [$minTime->format('Y-m-d'), $maxTime->format('Y-m-d')])->where('mprof_id', $user->id)->where('status_employee', 4)->join('m_profs', 'm_profs.id', '=', 'm_attends.mprof_id')->join('tmtables', 'tmtables.id', '=', 'm_attends.tmtable_id')->orderBy('m_profs.name', 'ASC')->get();
        $sick = MAttend::where('mprof_id', $this->uid)->whereBetween('date', [$minTime->format('Y-m-d'), $maxTime->format('Y-m-d')])->where('mprof_id', $user->id)->where('status_employee', 2)->join('m_profs', 'm_profs.id', '=', 'm_attends.mprof_id')->join('tmtables', 'tmtables.id', '=', 'm_attends.tmtable_id')->orderBy('m_profs.name', 'ASC')->get();
        ...
        return view('reportExportsSingle', [
            'final' => $final,
            'dates' => $dates,
            'date' => $date,
